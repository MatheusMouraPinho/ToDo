                    ...
                    @for($t=0; $t<sizeof($post['img_post']); $t++)
                      @if($post['img_post'][$t]->id_postagem == $id_post)
                        @if(Str::substr($post['img_post'][$t]->img_post, 30, 1) == 1)
                          <img class="popup_imgs" id="<?php echo 'img1'.$id_post ?>" onclick="show_modal('img1'+<?php echo $id_post ?>)" src="{{url($post['img_post'][$t]->img_post)}}">
                        @endif
                        @if(Str::substr($post['img_post'][$t]->img_post, 30, 1) == 2)
                          <img class="popup_imgs" id="<?php echo 'img2'.$id_post ?>" onclick="show_modal('img2'+<?php echo $id_post ?>)" src="{{url($post['img_post'][$t]->img_post)}}">
                        @endif
                        @if(Str::substr($post['img_post'][$t]->img_post, 30, 1) == 3)
                          <img class="popup_imgs" id="<?php echo 'img3'.$id_post ?>" onclick="show_modal('img3'+<?php echo $id_post ?>)" src="{{url($post['img_post'][$t]->img_post)}}">
                        @endif
                      @endif
                    @endfor 
                  </div> 
...
<div class="modal_imgs" id="img_modal" role="dialog">
  <div class="modal_content">
    <button type="button" class="close btnClose" onclick="hide_modal()">&times;</button>
...
<!--  Modal de avaliação  -->
<div class="modal fade bd-example-modal-lg" id="post<?php echo $id_post ?>_avaliar" tabindex="-1" role="dialog" id="modalideia" aria-hidden="true" style="background-color: rgb(0, 0, 0, .7);">
  <div class="modal-dialog modal-lg">
    <div class="modal-content">
...
@for($v=0; $v < sizeof($comments['comentarios']); $v++)
    @if($comments['comentarios'][$v]->id == $user && $comments['comentarios'][$v]->id_postagem == $id_post)
        ...
        <div class="modal fade id" id="popup{{ $comments['comentarios'][$v]->id_comentarios }}_apagar2" role="dialog" style="background-color: rgb(0, 0, 0, .7);">
            ...
            <div class="modal-content">
                ...
                <div class="modal-body"> 
                <p><h5>Deseja realmente apagar este comentário?</h5></p>
        ...
        <!-- Fim modal para apagar comentarios -->  
    ...
    @elseif($comments['comentarios'][$v]->id != $user && $comments['comentarios'][$v]->id_postagem == $id_post)
...
@for($n=0; $n < sizeof($comments['reply_coment']); $n++)
  @if($comments['reply_coment'][$n]->id == $user && $comments['reply_coment'][$n]->id_postagem == $id_post)
    <!--  Modal de edição de respostas de comentários -->
    <div class="painel-dados">
      <div class="modal fade id" id="popup{{$comments['reply_coment'][$n]->id_comentarios}}_edit" role="dialog" style="background-color: rgb(0, 0, 0, .7);">
        <div class="modal-dialog">
          <div class="modal-content">
            <div class="modal-header">
              <button type="button" class="close" data-dismiss="modal">&times;</button>
            ...
            <div class="modal-body"> 
              <form action="{{ route('edit.coment') }}" method="POST"> 
                ...
                <div class="div-edit">
                  <label style="vertical-align: top" for="editcomentario" class="bold subedit">Descrição:</label>
                  <textarea name="editcomentario" id="edit_desc" cols="60" rows="1" required maxlength="255">{{$comments['reply_coment'][$n]->conteudo_comentarios }}</textarea>
                  <input type="hidden" name="id_coment" value="{{ $comments['reply_coment'][$n]->id_comentarios }}">
    ...
    <div class="painel-dados">
      <div class="modal fade id" id="popup{{$comments['reply_coment'][$n]->id_comentarios}}_apagar1" role="dialog" style="background-color: rgb(0, 0, 0, .7);">
        <div class="modal-dialog">
          <div class="modal-content">
            <div class="modal-header">
            </div>
            <div class="modal-body"> 
              ...
              <div class="modal-footer">
                ...
                <form action="{{route('apagar-coment')}}" method="POST">
                  @csrf
                  <input name="id_comentario" type="hidden" value="{{ $comments['reply_coment'][$n]->id_comentarios }}">
    ...
    @elseif($comments['reply_coment'][$n]->id != $user && $comments['reply_coment'][$n]->id_postagem == $id_post)
      <!-- Modal denunciar comentario Reply -->
      <div class="modal fade id" id="den_comen_reply{{$comments['reply_coment'][$n]->id_comentarios }}" role="dialog" style="background-color: rgb(0, 0, 0, .7);">
        <div class="modal-dialog modal-content">
      ...
      $(function() {
        ...
        $('.modal')
          ...
          $('.modal_imgs')
            .on({
                'show.bs.modal': function() {
                    var idx = $('.modal_imgs:visible').length;
                    $(this).css('z-index', 1040 + (10 * idx));
                },
                'shown.bs.modal': function() {
                    var idx = ($('.modal_imgs:visible').length) - 1; // raise backdrop after animation.
                    $('.modal-backdrop').not('.stacked')
                    .css('z-index', 1039 + (10 * idx))
                    .addClass('stacked');
                },
                'hidden.bs.modal': function() {
                    if ($('.modal_imgs:visible').length > 0) {
                        // restore the modal-open class to the body element, so that scrolling works
                        // properly after de-stacking a modal.
                        setTimeout(function() {
                            $(document.body).addClass('modal-open');
